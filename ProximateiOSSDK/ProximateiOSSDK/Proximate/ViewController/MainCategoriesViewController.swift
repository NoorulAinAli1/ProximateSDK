...
class MainCategoriesViewController: UIViewController, CAPSPageMenuDelegate {
    ...
    override func viewDidLoad() {
        ...
        let btnSearch : UIBarButtonItem = UIBarButtonItem(image: UIImage(named: "button_search"), style: .Plain, target: self, action: #selector(MainCategoriesViewController.showSearchView(_:)))
        ...
        ApiHandler.sharedInstance.getAllCategories ({
            ...
            self.initializeTabs()
            DebugLogger.print("categories \(categories)")
        })
...
//        ]
    ...
    func showSearchView(sender: UIBarButtonItem){
        DebugLogger.print("showSearchView")
        
        if (navSearchView == nil) {
            navSearchView = NSBundle.mainBundle().loadNibNamed("SearchCategoryView", owner: self, options: nil)!.first as! SearchCategoryView
        ...
        DebugLogger.print("showSearchView \(navSearchView!)")
    ...
    func performSearch(sender : UIButton){
        if(navSearchView!.searchBar.text!.isEmpty){
            // show Error
            ProximateiOSSDK.getMessageDelegate()?.showMessage("toast_please_enter_some_search_criteria".localized)
        } else {
            let vC = controllerArray[(pageMenu?.currentPageIndex)!] as! CategoryTableViewController
            DebugLogger.print("vC \(vC)")
...
//            .SelectedMenuItemLabelColor(UIColor.orangeColor())
    ...
    func didMoveToPage(controller: UIViewController, index: Int) {
        DebugLogger.print("didMoveToPage \(controller)")
    ...
    func willMoveToPage(controller: UIViewController, index: Int) {
        DebugLogger.print("willMoveToPage \(controller)")
